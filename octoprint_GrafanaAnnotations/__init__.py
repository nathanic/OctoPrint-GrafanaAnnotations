...
class GrafanaAnnotationPlugin(octoprint.plugin.EventHandlerPlugin,
    ...
    def begin_print_annotation(self, payload):
        ...
        ann = {'text': 'Print of %s' % payload['path'],
                ...
                'tags': ['printjob', self.get_printer_tag() ] 
                }
        self._logger.info('kilroy was here')
        ...
        response = requests.post(self.get_api_url(), json=ann, headers=self.headers(), timeout=TIMEOUT)
    ...
    def end_print_annotation(self, success, payload):
        if self.currentAnnotationId is None:
            self._logger.warn("Can't update grafana annotation because we don't have a current ID")
            return

        end_time = time.time()
        tags = ['printjob', self.get_printer_tag()]
        if success:
            tags.append('printjob_succeeded')
        else:
            tags.append('printjob_failed')
        ...
        response = requests.patch(url, headers=self.headers(), json=ann, timeout=TIMEOUT)
        self.currentAnnotationId = None
    ...
    def get_printer_tag(self):
        # minimal effort
        #return 'printer_' + self._settings.global_get(['appearance','name']).replace(' ', '_').lower()
        return 'printer_test'

def unixtime_to_javatime(t):
    return int(round( t * 1000))

__plugin_implementation__ = GrafanaAnnotationPlugin()
